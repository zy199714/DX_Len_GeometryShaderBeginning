...
bool ObjReader::ReadMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		// [材质]64字节
		fin.read(reinterpret_cast<char*>(&objParts[i].material), sizeof(material_struct));
...
bool ObjReader::WriteMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		// [材质]64字节
		fout.write(reinterpret_cast<const char*>(&objParts[i].material), sizeof(material_struct));
