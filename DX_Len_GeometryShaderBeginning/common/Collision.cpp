...
Ray Ray::ScreenToRay(const GameCamera & camera, float screenX, float screenY)
{
	// ******************
	...
	// 从NDC坐标系变换回世界坐标系
	XMMATRIX Transform = XMMatrixMultiply(camera.ViewMatrix(), camera.PerspectiveProjectionMatrix());
	...
	// 求出射线
	XMFLOAT3 direction;
	XMStoreFloat3(&direction, XMVector3Normalize(Target - camera.PositionVector()));
